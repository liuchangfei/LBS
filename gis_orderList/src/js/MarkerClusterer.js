...
(function(){
    ...
    var getLimitLenText = function(content, maxChars) {
    	if(!content) {
    		return "";
    	}
    ...
    var MarkerClusterer =  
         ...
         *    data {Array<Marker>|Object{provinceItems:provinceItems, cityItems:cityItems, items:items}} 要聚合的标记数组或者对象<br />
         *    girdSize {Number} 聚合计算时网格的像素大小，默认60<br />
         *    maxZoom {Number} 最大的聚合级别，大于该级别就不进行相应的聚合，默认18<br />
        ...
        BMapLib.MarkerClusterer = function(map, options){
            ...
            if(isArray(data)) {
            	...
            	this._isObjectData = true;
                this.redrawData();
            }
            
            var that = this;
            this._map.addEventListener("zoomend",function(event){
                that._isObjectData ? that.redrawData() : that._redraw();     
            });
    ...
    MarkerClusterer.prototype.redrawData = function(){
    ...
    MarkerClusterer.prototype.getIcon = function(iconUrl, size, anchor) {
        var icon;
    ...
    MarkerClusterer.prototype.addProvinceMarkers = function() {
    	var map = this._map;
    	var data = this._data;
    	var items = data.provinceItems;
    	var icon = this.getIcon("assets/images/circle_green.png", [72, 72], [72/2, 72/2]);
    	var labelStyle = {backgroundColor:"none", color:"#fff", font:"微软雅黑", fontSize:"13px", border:"none"};
    	for(var i = 0, len = items.length; i < len; i++) {
    		var item = items[i];
    		var pt = new BMap.Point(item.lng, item.lat); 
    		var marker = new BMap.Marker(pt); 
    		marker.setIcon(icon);
    		marker.setTitle("省份：" + item.name + "\n数量：" + item.count);
    		var label = new BMap.Label("<div style='text-align:center;width:72px;height:49px;padding-top:19px;'>" + getLimitLenText(item.name, 4) + "<br/>" + item.count + "</div>");
    ...
    MarkerClusterer.prototype.addCityMarkers = function() {
        var map = this._map;
        var data = this._data;
        var items = data.cityItems;
        var icon = this.getIcon("assets/images/circle_orange.png", [72, 72], [72/2, 72/2]);
        var labelStyle = {backgroundColor:"none", color:"#fff", font:"微软雅黑", fontSize:"13px", border:"none"};
        for(var i = 0, len = items.length; i < len; i++) {
            var item = items[i];
            var pt = new BMap.Point(item.lng, item.lat); 
    ...
    MarkerClusterer.prototype.addItemMarkers = function() {
        var map = this._map;
        var data = this._data;
    ...
    MarkerClusterer.prototype._addToClosestCluster = function (marker){
        ...
        for(var i = 0, cluster; cluster = clusters[i]; i++){
            ...
            if(center){
                var d = map.getDistance(center, position);
                if(d < distance){
                    distance = d;
     ...
     * @return void。
     */
	MarkerClusterer.prototype.updateAllClusters = function() {
		for(var i = 0, cluster; cluster = this._clusters[i]; i++) {
    ...
    Cluster.prototype.addMarker = function(marker){
        ...
        this._map.addOverlay(this._clusterMarker);
		this._isReal = true;
        //this.updateClusterMarker();//每次addMarker都刷新，效率低
        return true;
    };
    ...
    Cluster.prototype.updateClusterMarker = function () {
        ...
        this._clusterMarker.onclick = function(){
