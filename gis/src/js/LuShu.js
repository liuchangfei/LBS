...
(function() {
     ...
     */
     BMapLib.LuShu = function(map, path, opts) {
        ...
        this.routeControl = function(value) {
            if (!arguments.length) return routeControl;
            routeControl = value;
            return this;
        };
        ...
        this.treeControl = function(value) {
            if (!arguments.length) return treeControl;
            treeControl = value;
            return this;
        };
     ...
     * @return 无返回值.
     */
    LuShu.prototype.destroy = function() {
    	var me = this;
    	me._removeAll();
    }
    ...
    LuShu.prototype.start = function() {
        ...
        if (me.i && me.i < len - 1) {
        	...
        	//滚动时间轴到起点对应的check point
        	//me.routeControl().scrollToCheckPoint(0);
        	//滚动树到对应的节点
        	me.treeControl().scrollToCheckPoint(0);
    ...
    /**
     * 移动到索引值对应的点
     * @param index 要移动到的点对应的索引值
     * @return 无返回值.
     */
    LuShu.prototype.moveTo = function(index) {
    	var me = this;
    	me.i = index;
    	var pos = me._path[index];
    	me._moveMarker(pos);
    	//地图移动到以目标点为中心
    	me._map.panTo(pos);
    	clearInterval(me._intervalFlag);
...
//		me._moveNext(index);
    ...
    baidu.object.extend(LuShu.prototype, {
        ...
        _moveMarker: function(pos) {
            ...
            //设置自定义overlay的位置
            this._setInfoWin(pos);
        },
        /**
         * 移除现有的marker和overla,并停止播放
         * @return 无返回值.
         */
        _removeAll: function() {
        	if (this._marker) {
...
//            }
            var deg = Math.atan2(targetPos.y-curPos.y, targetPos.x-curPos.x)*180/Math.PI;
            me._marker.setRotation(deg); 
            return;
        ...
        _pauseForView: function(index) {
            var me = this;
            //滚动时间轴到当前对应的check point
            //me.routeControl().scrollToCheckPoint(index);
            //滚动树到当前对应的节点
            me.treeControl().scrollToCheckPoint(index);
        ...
        _troughPointIndex: function(markerPoi) {
            var distance, me = this, t = this._opts.landmarkPois;
            for (var p, i = me.i, len = t.length; i < len; i++) {
                //用me.i当索引，前面从最新停靠的索引点开始搜索
            	p = t[i];
                distance = this._map.getDistance(new BMap.Point(p.lng, p.lat), markerPoi);
                //两点距离小于10米，认为是同一个点
                if (distance < 10) {
                    return i;
                }
    ...
    CustomOverlay.prototype.initialize = function(map) {
        var div = this._div = baidu.dom.create('div', {style: 'border:1px solid rgba(20,255,255,0.5);width:auto;min-width:50px;text-align:left;position:absolute;background:rgba(68,123,175,0.75);color:#fff;font-size:12px;padding:5px;white-space: nowrap;'});
